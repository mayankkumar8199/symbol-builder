import os, re, glob, shutil, tkinter as tk
...
class SymbolPalette(ttk.Frame):
    ...
    def load_folder(self, folder: str):
        ...
        for idx, path in enumerate(self.files, 1):
            ...
            def begin(ev, n=name, p=path): self.on_start_drag(n, p, ev)
            lbl_img.bind("<Button-1>", begin); lbl_txt.bind("<Button-1>", begin)
...
class BoardCanvas(tk.Canvas):
    def __init__(self, master, **kw):
        ...
        self.hint = self.create_text(CANVAS_SIZE[0]//2, CANVAS_SIZE[1]//2,
    ...
    def place_symbol(self, name, img_path, x, y, base_px=160):
        ...
        w = max(1, int(pil.width * scale)); h = max(1, int(pil.height * scale))
        ...
        if self.hint: self.delete(self.hint); self.hint = None
    ...
    def _on_click(self, ev):
        ...
        if hit and self.type(hit) == "image":
            ...
            self._update_selection(hit[0]); return
    ...
    def _on_release(self, ev): self._drag["item"] = None

    def _update_selection(self, cid_or_none):
        for it in self.find_withtag("selbox"): self.delete(it)
    ...
    def _delete_selected(self, ev=None):
        if self.selected_id and self.selected_id in self.placed:
            self.delete(self.selected_id); del self.placed[self.selected_id]
            self.selected_id = None
            if not self.find_all() and not self.hint:
                self.hint = self.create_text(CANVAS_SIZE[0]//2, CANVAS_SIZE[1]//2,
                    text="Drag from palette → drop here", fill="#8b8b8b", font=("Segoe UI", 14, "italic"))
            self.event_generate("<<SymbolRemoved>>"); self.event_generate("<<SelectionChanged>>")

    def clear_board(self):
        for cid in list(self.placed.keys()): self.delete(cid)
        self.placed.clear(); self.selected_id = None
        if not self.hint:
            self.hint = self.create_text(CANVAS_SIZE[0]//2, CANVAS_SIZE[1]//2,
                text="Drag from palette → drop here", fill="#8b8b8b", font=("Segoe UI", 14, "italic"))
        self.event_generate("<<SymbolRemoved>>"); self.event_generate("<<SelectionChanged>>")
    ...
    def resize_selected(self, factor):
        ...
        rec["scale"] = new_scale; self._apply_scale(cid, rec)

    def set_selected_scale_abs(self, scale_abs):
        cid = self.selected_id
        if not cid or cid not in self.placed: return
        rec = self.placed[cid]; rec["scale"] = max(0.2, min(4.0, scale_abs))
    ...
    def duplicate_selected(self):
        cid = self.selected_id
        if not cid or cid not in self.placed: return
        rec = self.placed[cid]; x, y = self.coords(cid)
        nid = self.place_symbol(rec["name"], rec["path"], x + 25, y + 25, base_px=max(rec["pil"].size))
        self.placed[nid]["scale"] = rec["scale"]; self._apply_scale(nid, self.placed[nid])
...
class Inspector(ttk.Frame):
    def __init__(self, master, board: BoardCanvas, **kw):
        super().__init__(master, **kw)
        self.board = board
        self._suspend_slider_cb = False  # <<< guard to stop recursion
        ...
        ttk.Button(btns, text="Delete", command=self.board._delete_selected).pack(side="left", padx=2)
    ...
    def _on_slider(self, _val):
        if self._suspend_slider_cb:  # <<< prevent feedback loop
            return
    ...
    def refresh(self, ev=None):
        ...
        if self.board.selected_id and self.board.selected_id in self.board.placed:
            ...
            self._suspend_slider_cb = True  # <<< guard during programmatic set
            try:
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Symbol Builder (folder-based + resize + upload)")
        ...
        left.pack(side="left", fill="y"); center.pack(side="left", fill="both", expand=True); right.pack(side="right", fill="y")
        ...
        if not os.path.isdir(folder):
            ...
            if not folder:
                messagebox.showerror("No folder", "No folder selected. Exiting."); self.destroy(); return
    ...
    def _choose_folder(self):
        ...
        self.current_folder = folder; self.palette.load_folder(folder)
        self.status.configure(text=f"Folder: {self.current_folder}")

    def _reload(self):
        if not hasattr(self, "current_folder"): self._choose_folder(); return
    ...
    def _upload_symbols(self):
        ...
        paths = filedialog.askopenfilenames(title="Select image files to add",
                                            filetypes=[("Images", "*.png;*.jpg;*.jpeg;*.webp;*.bmp")])
        if not paths: return
        copied = 0
        for p in paths:
            try: safe_copy_to_folder(p, self.current_folder); copied += 1
            except Exception as e: messagebox.showwarning("Copy failed", f"Could not add {os.path.basename(p)}:\n{e}")
        if copied:
            self.palette.load_folder(self.current_folder)
            self.status.configure(text=f"Uploaded {copied} file(s) → palette refreshed")

    def _clear_board(self): self.board.clear_board()
    def _on_palette_drag_start(self, name, img_path, event): DragGhost(self, name, img_path, on_drop=self._on_drop_to_canvas)
    def _on_drop_to_canvas(self, canvas: BoardCanvas, name, img_path, x, y):
        canvas.place_symbol(name, img_path, x, y); self.status.configure(text=f"Placed: {name}")
